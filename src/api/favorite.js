...
function removeFavorite(token, attractionId, memberId, success, fail) {
  local
    .delete(`/favorite?attractionId=${attractionId}&memberId=${memberId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
function selectFavorite(token, attractionId, memberId, success, fail) {
  local
    .get(`/favorite/attraction?attractionId=${attractionId}&memberId=${memberId}`, {
      headers: {
        Authorization: `Bearer ${token} `,
      },
    })
...
// 멤버의 즐겨찾기 조회
function selectFavoritesByMemberId(memberId, success, fail) {
  local.get(`/favorite/user?memberId=${memberId}`).then(success).catch(fail);
}
