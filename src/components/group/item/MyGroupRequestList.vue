    ...
    <template v-for="request in groupRequests" :key="request.id">
      <div class="row request">
        <div class="col-lg-9 member-info d-flex align-items-center">
          <img v-if="request.memberProfileImage !== null" :src="request.memberProfileImage" alt="Profile Image" class="profile-image">
        ...
        <div class="col-lg-3 request-btn d-flex flex-column justify-content-center">
          <button class="btn accept-btn mb-2" @click.prevent="acceptRequestFunc(request.memberId)">수락</button>
          <button class="btn refuse-btn" @click.prevent="refuseRequestFunc(request.memberId)">거절</button>
        </div>
      </div>
    </template>
